...
var SQLiteProxy = (function() {
    ...
    for (field in filters) {
      ...
      for (prop in filters[field]) {
        switch (prop) {
          ...
          case "$start":
            where +=
              [field, " LIKE '", filters[field][prop], "%'"].join("") + " AND ";
            break;
          case "$end":
            where +=
              [field, " LIKE '%", filters[field][prop], "'"].join("") + " AND ";
          ...
          case "$contain":
            where +=
              [field, " LIKE '%", filters[field][prop], "%'"].join("") +
              " AND ";
            break;
          case "$in":
            where +=
              [field, " IN (", filters[field][prop].join(","), ")"].join("") +
              " AND ";
            break;
          default:
            where += "";
  ...
  var _formatGroupBy = function(key, options, groups, filters) {
    var opts = options && options instanceof Array ? options : [options],
      ...
      where =
        filters && typeof filters === "object" ? _formatSql("", filters) : "",
      sql = "",
      prop,
      field,
      alias;
...
if (typeof module === "object" && module.exports) {
  module.exports.SQLiteProxy = SQLiteProxy;
}
