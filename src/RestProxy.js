  ...
  var _httpRequest = function(url, method, config, success, error) {
    ...
    http.onreadystatechange = function() {
      if (http.readyState === 4) {
        callback =
          [200, 201, 204, 304].indexOf(http.status) > -1 ? success : error;
        callback(http);
      }
    };
    ...
    if (opts.params) {
      for (p in opts.params) {
        url +=
          p +
          "=" +
          (typeof opts.params[p] === "object"
            ? JSON.stringify(opts.params[p])
            : opts.params[p]) +
          "&";
      }
    }
...
if (typeof module === "object" && module.exports) {
  module.exports.RestProxy = RestProxy;
}
